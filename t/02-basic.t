...
use Dancer::Session::Redis::Simple;
...
my $session;
lives_ok { $session = Dancer::Session::Redis::Simple->create } 'Session engine created okay';

isa_ok  $session, 'Dancer::Session::Redis::Simple', 'Engine was blessed correctly';
...
# unknown session-id
my $s1 = Dancer::Session::Redis::Simple->retrieve('XXX');
is      $s1,    undef,                      'Unknown session cannot be found';

# known session-id
my $s2 = Dancer::Session::Redis::Simple->retrieve($sid);
isa_ok  $s2,    'Dancer::Session::Redis::Simple',   'Session object was blessed correctly';
